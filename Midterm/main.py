...
#AI which is player 2 wants to maximize the score
#Human which is player 1 wants to minimize the score
...
def best_move():
    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLS):
            if board[row][col] == 0:
                board[row][col] = 2
                score = minimax(board, 0, is_maximizing=False) #is_maximizing is False because the next turn is minimizing player which is player 1 human
                board[row][col] = 0
                if score > best_score:
                    best_score = score
                    move = (row, col)
